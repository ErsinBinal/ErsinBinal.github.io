...
| `home-protocol.js` içindeki komut tanımı | 132 | 109 | 95 | 95 | 95 | 95 | 95 | 95 | 95 |
| `route-commands.js` içindeki komut | yok | 23 | 23 | 23 | 23 | 23 | 23 | 23 | 23 |
| `guide-commands.js` içindeki komut | yok | yok | 14 | 14 | 14 | 14 | 14 | 14 | 14 |
| `vfs.js` | yok | yok | yok | 132 satır | 205 satır | 205 satır | 205 satır | 205 satır | 205 satır |
| `world.js` | yok | yok | yok | yok | yok | 234 satır | 234 satır | 234 satır | 234 satır |
| `world-actions.js` | yok | yok | yok | yok | yok | yok | 169 satır | 169 satır | 169 satır |
| `economy.js` | yok | yok | yok | yok | yok | yok | yok | 74 satır | 74 satır |
...
### Faz 3A — Shard ekonomi çekirdeği — TAMAMLANDI; CANLI
...
- Yerel smoke 8/8; Playwright E2E 7/7 geçti. Gerçek kullanıcı oluşturan kayıt
  testi bilinçli olarak skip edildi.
- Kullanıcı yayını sonrası canlı ana sayfada `economy.js?v=1`,
  `world-actions.js?v=1`, `vfs.js?v=2`, `home-protocol.js?v=81` ve
  `convivium-v203` doğrulandı. Economy SHA-256
  `15aaaa7a2cdcd730503f4b79b9c0d0a463fcd790c5d1ccd0846d458503d4a89c`,
  protocol SHA-256
  `526e6bbc1698c0bfac299bfd7d3365572673f85e0931dc0dd9e80318f70ae52c`
  olarak main `7a24c3c` ile birebir eşleşti.
...
- Faz 3A canlı kabulü tamamlandı; çalışma ağacı temiz main `7a24c3c` üzerinden
  açıldı.
- Mevcut dört ürünün katalog sırası, anahtar/fiyat/açıklaması immutable
  `shopCatalog` verisine; listeleme, sahiplik, bakiye guard'ları ve satın alma
  kararı ayrı `createShop(deps)` factory'sine taşındı.
- `theme-magenta`/`theme-ice` envanter tüketimi, `saver-drift` storage bayrağı
  ve screen-saver tüketicisi korundu. `bugy-flag` storage yazımı da değişmeden
  kaldı; repoda bu bayrağı okuyan bir çalışma zamanı tüketicisi bulunmadığı
  ayrıca kaydedildi.
- Factory localStorage, Supabase, DOM, audio veya protocol state'ine doğrudan
  erişmiyor. Bakiye/envanter getter-yazımı, persist/cloud save, kozmetik storage
  ve pickup sesi protocol callback'lerinde kaldı.
- Economy modülü yoksa eski `shop: economy unavailable` davranışı korunuyor.
  Yalnız shop asset'i yoksa `shop: unavailable` dönüyor; economy, VFS ve diğer
  terminal alanları çalışmayı sürdürüyor.
- `shop.js?v=1`, `home-protocol.js?v=82` ve `convivium-v204` atomik yayın
  dilimi olarak bağlandı. Shop asset'i syntax/precache/validator/cache-sync
  sözleşmelerine alındı; managed asset sayısı 25 oldu.
- Bu dilimde kart/collect, fiyatlar, ürün metinleri, Supabase şeması ve parser
  değiştirilmedi.

Karakterizasyon ve doğrulama:

- Yeni `tests/unit/home-shop.test.mjs` 6/6: katalog snapshot/immutability,
  liste/sahiplik formatı, kullanım-bilinmeyen-yetersiz bakiye guard'ları,
  başarılı satın alma çağrı sırası/çıktı/ipucu, eksik dependency ve sahiplik
  sınırı. Global unit toplamı 53/53'e çıktı; 132 komut korundu.
- Normal Chromium akışında tema, saver ve Bugy bayrağı alındı; 60 bakiyeden
  sırasıyla 48/28/20'ye geçildi. Tema CSS uygulaması, iki localStorage bayrağı,
  envanter ve reload kalıcılığı doğrulandı; page error oluşmadı.
- Shop asset'i engellendiğinde açık fallback mesajı görüldü; `shards` bakiyesi
  ve `cd notes` VFS akışı çalıştı, shard/envanter değişmedi ve page error yoktu.
- Service Worker kontrollü çevrimdışı reload'da `convivium-v204` ile shop satın
  alma, `[x]` sahiplik işareti ve bakiye kalıcılığı doğrulandı; page error yok.
...
| Dosya | Amaç |
|---|---|
| `assets/js/home/shop.js` | Immutable katalog ve shop karar factory'si |
| `assets/js/home-protocol.js` | Shop kurulumu, state/effect callback'leri ve ince delegasyon |
| `tests/unit/home-shop.test.mjs` | Katalog/guard/sıra/fallback/sahiplik karakterizasyonu |
| `index.html` | Shop v1 yükleme sırası ve protocol v82 |
| `service-worker.js` | Shop precache ve cache v204 |
| `scripts/validate-site-integrity.js` | Shop asset/factory/sıra/sürüm sözleşmesi |
| `scripts/sync-cache-versions.js` | Shop asset'ini managed listeye alma |
| `package.json` | Shop modülünü syntax kapısına alma |
...
Sorun görülürse canlı Faz 3A sınırını koruyarak yalnız Faz 3B geri alınır:

1. Shop katalog, sahiplik/listeleme guard'ları ve satın alma kararını aynı
   çıktı/çağrı sırasıyla protocol içindeki eski konumuna geri koy.
2. Economy/world/VFS sınırlarını yerinde bırak; `spendShards` delegasyonunu veya
   kozmetik storage/theme/screen-saver tüketicilerini değiştirme.
3. `home-shop.test.mjs` davranış testlerini inline uygulamayı doğrulayacak
   biçimde koru; katalog snapshot'ını ve kozmetik tüketici kontrollerini silme.
4. `shop.js` script ve syntax/precache/validator/cache-sync bağlantılarını
   kaldır. Protocol asset ve Service Worker cache sürümünü yeni ileri değerlere
   bump et; `convivium-v204` yayımlandıysa hiçbir koşulda v203'e düşürme.
