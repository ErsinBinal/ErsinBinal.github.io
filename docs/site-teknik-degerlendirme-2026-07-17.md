...
| Terminal monolit Faz 2C | Kod/test tamam; yayın bekliyor | Sekiz odalık immutable world registry + görünüm/inceleme modeli ayrıldı; mutasyonlar yerinde; SW v201 yerel |
...
Canlı Faz 2B kabulünden sonra yerel Faz 2C'de sekiz odanın immutable registry'si,
panel/çıkış/görev/unvan görünümü ve `look/examine` davranışı 234 satırlık
`world.js` factory'sine ayrıldı. `take/unlock/use`, state yazımı, ödül/ekonomi
ve Supabase yan etkileri protokolde bırakıldı. `home-protocol.js` 4.530'dan
4.113 satıra indi; `vfs.js` 205 satır. Global karakterizasyon
...
önlüyor. Faz 2C canlı kabulünden sonraki güvenli aday, `take/unlock/use` yan
etkilerini test-first karakterize edip ekonomi ve Supabase semantiğini
değiştirmeden ayrı mutasyon karar sınırına almaktır.
...
| Kontrol | Sonuç |
|---|---|
| `npm run check` | Geçti; unit 31/31, Worker 12/12, 27/27 CSP, 22/22 harici script tam sürüm |
...
- [ ] VFS/world domainlerini ayır (Faz 2A navigation ve Faz 2B kalıcı `/home`
  canlı; Faz 2C salt okunur world modeli yerel doğrulandı; mutasyon komutları
  sırada).
