...
| Terminal monolit Faz 2D | Kod/test tamam; yayın bekliyor | `take/unlock/use` kararları callback tabanlı world-actions factory'sine ayrıldı; yan etki sahipliği protocol'de; SW v202 yerel |
...
ayrıldı. Yerel Faz 2D'de `take/unlock/use` doğrulama ve karar ağacı 169 satırlık
...
kaldı. `home-protocol.js` 4.530'dan 4.058 satıra indi; `vfs.js` 205 satır.
...
değişmiyor. Faz 2D canlı kabulünden sonraki güvenli aday, shard ekonomisinin
localStorage/Supabase fallback ve çağrı sırasını test-first karakterize ederek
ayrı bir sahiplik sınırı kurmaktır.
...
| Kontrol | Sonuç |
|---|---|
| `npm run check` | Geçti; unit 37/37, Worker 12/12, 27/27 CSP, 22/22 harici script tam sürüm |
...
| Zorunlu JS/MJS syntax kapısı | Geçti; yeni world-actions modülü dahil |
...
- [x] VFS/world domainlerini ayır (Faz 2A/2B VFS ve Faz 2C world read-model
  canlı; Faz 2D mutasyon karar sınırı yerel doğrulandı ve yayın bekliyor).
- [ ] Ekonomi sahipliğini ayır (önce shard localStorage/Supabase fallback ve
  ödül/harcama çağrı sırasını karakterize et).
